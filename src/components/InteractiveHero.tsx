...
export function InteractiveHero() {
  ...
  return (
    <div className="relative">
      ...
      <section className="relative overflow-hidden rounded-[6px] bg-[#2f4c73] px-5 py-8 text-white shadow-[0_24px_70px_rgba(31,49,78,0.18)] md:px-8 md:py-12">
      ...
      <div className="grid gap-10 lg:grid-cols-[1.08fr_0.92fr] lg:items-stretch">
        ...
        <motion.div
          className="flex h-full items-stretch justify-center lg:justify-end"
          initial={reduceMotion ? false : { opacity: 0, scale: 0.98 }}
          animate={reduceMotion ? undefined : { opacity: 1, scale: 1 }}
          transition={{ delay: 0.15, duration: 0.65, ease: [0.22, 1, 0.36, 1] }}
        >
          <div className="flex w-full max-w-xl flex-col justify-between gap-6">
            <div className="flex flex-wrap items-center justify-center gap-2.5 lg:justify-start">
              {siteConfig.stackBands.map((tech) => (
                <ProjectTechBadge
                  ...
                  tech={tech}
                  compact
                  className="border-white/18 bg-white/90 px-2.5 py-1.5 text-[9px] text-black shadow-[0_10px_24px_rgba(15,23,42,0.12)] backdrop-blur xl:px-3 xl:text-[10px]"
                />
              ))}
            </div>
              ...
              {proofMetrics.map((metric, index) => (
                ...
                >
                  ...
                  <div className="mt-3 flex items-baseline gap-3">
                    ...
                    <span className="min-w-0 whitespace-nowrap text-sm leading-6 text-slate-300">{metric.detail}</span>
                  </div>
                </motion.div>
              ))}
            </div>
