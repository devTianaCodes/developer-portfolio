...
export default function AboutPage({ params }: AboutPageProps) {
  ...
  const featuredProjectLinks = [
    { label: t("chocolateDemo"), href: getProjectBySlug("chocolate")?.links.find((link) => link.kind === "live")?.href },
    { label: t("petNestDemo"), href: getProjectBySlug("petnest")?.links.find((link) => link.kind === "live")?.href }
  ];
          ...
          <div className="sharp-panel p-8 lg:order-2">
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-3 md:justify-start">
              {featuredProjectLinks.map((projectLink) => (
                projectLink.href ? <Link key={projectLink.href} href={projectLink.href} target="_blank" rel="noreferrer" className="sharp-button">
                  {projectLink.label}
                </Link> : null
              ))}
