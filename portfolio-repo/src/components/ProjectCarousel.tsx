...
function ProjectPanel({ project, isHovered, reduceMotion, transitionMs, copy }: ProjectPanelProps) {
      ...
      >
        {project.imageSrc ? (
          <motion.div
            ...
            animate={{
              scale: reduceMotion ? 1 : isHovered ? 1.035 : 1.01,
              y: !reduceMotion && isHovered ? -5 : 0
            }}
            transition={{
...
export function ProjectCarousel({ projects }: ProjectCarouselProps) {
  ...
  const focusActiveProjectRef = useRef(false);
  ...
  }, [autoplayEnabled, carouselInView, pageVisible, pointerInside, projects.length, reduceMotion]);
  ...
  useEffect(() => {
    if (focusActiveProjectRef.current) {
      focusActiveProjectRef.current = false;
      carouselRef.current?.querySelector<HTMLAnchorElement>('[data-active="true"] a')?.focus({ preventScroll: true });
    }
  ...
  function handleKeyDown(event: React.KeyboardEvent<HTMLElement>) {
    ...
    focusActiveProjectRef.current = event.target instanceof HTMLAnchorElement;
    move(direction);
  }
  ...
  return (
    <section
      ...
      onFocusCapture={(event) => {
        // Resume remains an explicit choice after keyboard focus enters the carousel.
          ...
          {projects.map((project, index) => {
            ...
            return (
              <motion.div
                ...
                animate={{
                  opacity: 1,
                  scale: !reduceMotion && isHovered ? 1.015 : 1,
                  y: !reduceMotion && isHovered ? (isActive ? -4 : -7) : 0,
                  z: isMobileViewport
                    ? 0
                    : isActive
                      ? isHovered
              ...
              >
                <ProjectPanel
                  ...
                  transitionMs={reduceMotion ? 0 : transitionMs}
                  copy={panelCopy}
                />
                {isActive ? (
                  <Link
                ...
                <motion.div
                  ...
                  transition={{
                    duration: reduceMotion ? 0 : isShadeFree ? shadeRemovalSeconds : shadeApplicationSeconds,
                    ease: carouselEase
                  }}
                />
              </motion.div>
